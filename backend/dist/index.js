...
const database_1 = require("./database"); // Assuming the DB connection is in database.ts
const Loan_1 = require("./models/Loan"); // Import the Loan model
...
const errorHandler = (res, statusCode, message, details) => {
...
app.post('/loans', asyncWrapper((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const newLoan = new Loan_1.Loan({ idNumber, fullName, loanAmount, loanTenure, employmentStatus, reasonForLoan, employmentAddress, status, loanOfficer });
        yield newLoan.save();
        res.status(201).json(newLoan); // Send back the created loan data
    }
    catch (error) {
        errorHandler(res, 500, 'Error creating loan entry', error.message);
    }
})));
// API to get loan entries based on user role (verifier or admin)
app.get('/loans', asyncWrapper((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (role === 'verifier') {
            ...
            loans = yield Loan_1.Loan.find({ status: { $in: ['PENDING', 'VERIFIED'] } });
        }
        else if (role === 'admin') {
            // For admin: Get loans where status is "PENDING" or "APPROVED"
            loans = yield Loan_1.Loan.find({ status: { $in: ['VERIFIED', 'APPROVED', "REJECTED"] } });
        }
        else {
        ...
        errorHandler(res, 500, 'Error fetching loan entries', error.message);
    }
})));
// API to get loan entries by idNumber
app.get('/loans/id', asyncWrapper((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { idNumber } = req.query;
    if (!idNumber || isNaN(Number(idNumber))) {
        return errorHandler(res, 400, 'Invalid idNumber provided. It should be a number.');
    }
    try {
        const loans = yield Loan_1.Loan.find({ idNumber: Number(idNumber) }); // Convert idNumber to a number
        if (loans.length === 0) {
        ...
        errorHandler(res, 500, 'Error fetching loans by idNumber', error.message);
    }
})));
// API to update the loan status using the loan's Object ID
app.patch('/loans/status-verifier', asyncWrapper((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { _id } = req.query;
    ...
    try {
        const updatedLoan = yield Loan_1.Loan.findByIdAndUpdate(_id, { status, loanOfficer }, { new: true, runValidators: true });
        if (!updatedLoan) {
        ...
        errorHandler(res, 500, 'Error updating loan status', error.message);
    }
})));
// API to update the loan status by Admin using the loan's Object ID
app.patch('/loans/status-admin', asyncWrapper((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { _id } = req.query;
    ...
    try {
        const updatedLoan = yield Loan_1.Loan.findByIdAndUpdate(_id, { status }, { new: true, runValidators: true });
        if (!updatedLoan) {
            return errorHandler(res, 404, 'Loan not found.');
        }
        res.status(200).json(updatedLoan);
...
app.get('/loans/summary', asyncWrapper((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Perform all operations in parallel using Promise.all
        const [distinctActiveUsers, distinctBorrowUsers, approvedLoanCount, totalDisbursed] = yield Promise.all([
            // 1. Get the number of distinct active users by idNumber
            Loan_1.Loan.aggregate([
                { $group: { _id: "$idNumber" } }
            ]),
            ...
            Loan_1.Loan.countDocuments({ status: 'APPROVED' }),
        ...
        errorHandler(res, 500, 'Error fetching loan summary', error.message);
    }
})));
